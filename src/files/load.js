!function() {
   ...
   load.parseFileName = (filename) => {
      ...
      if (filename[0] == 'R') {
         ...
         if (datestring.length == 8) {
            ...
            if (date == xyz.util.formatDate(date)) meta.filedate = date;
         }
      }

      if (parts.length == 5) {
         // using charCodeAt as safety since Diacritics has sometimes failed
         let gender = xyz.util.replaceDiacritics(parts[3]).toUpperCase().charCodeAt(0);
   ...
   load.importPlayerList = (rows, id) => {
      ...
      rows.forEach(player => {
         ...
         if (name) {
            ...
            player.hash = xyz.util.nameHash(name);
            player.birth = determineDate(player.birth);
            player.first_name = xyz.util.normalizeName(player.first_name, false);
            player.last_name = xyz.util.normalizeName(player.last_name, false);
   ...
   function importClubsCSV(clubs) {
      ...
      xyz.util.performTask(db.addClub, clubs, false).then(() => gen.busy.done(id), () => gen.busy.done(id));
   }
   ...
   function importRankings(rows) {
      return new Promise((resolve, reject) => {
         ...
         let categories = xyz.util.unique(rows.map(r=>config.legacyCategory(r.category)));
         ...
         xyz.util.performTask(db.addCategoryRankings, category_rankings, false).then(done, done);
   ...
   function importTournaments(rows) {
      ...
      rows.forEach(record => { 
         if (record.tuid) {
            ...
            let tournament = {
               ...
               name: xyz.util.normalizeName(record.name, false),
      ...
      xyz.util.performTask(db.addTournament, tournaments, false).then(done, done);
   ...
   load.addNewTournaments = (trnys) => {
      ...
      console.log(id);
      xyz.util.performTask(db.addTournament, trnys, false).then(done, () => gen.busy.done(id));
   ...
   function processPlayers(players) {
      ...
      return new Promise((resolve, reject) => {
         ...
         xyz.util.performTask(processPlayer, data)
            .then(results => {
   ...
   function identifyPlayers({ completed, outstanding }) {
      ...
      actions_element
         ...
         .on('click', (d, i, elem) => { 
            ...
            let row = xyz.util.getParent(elem[i], 'section_row');
      ...
      let dups = outstanding.filter(action => action.status == 'duplicate')
         .filter(action => {
            ...
            let valid = action.result.filter(result => {
               ...
               return xyz.util.isMember(eligible_categories, load.loaded.tournament.category);
            });
            if (valid.length == 1) {
   ...
   function loadTask(fx, arr, what = '', callback) {
      ...
      xyz.util.performTask(fx, Array.isArray(arr) ? arr : [arr], false).then(finish, finish);
   ...
   function loadWorkbook(file_content, callback) {
      ...
      if (workbook_type == 'courthive_imports') {
         ...
         let addPlayers = () => new Promise((resolve, reject) => xyz.util.performTask(db.addPlayer, players, false).then(resolve, resolve));
         let addTournaments = () => new Promise((resolve, reject) => xyz.util.performTask(db.addTournament, tournaments, false).then(resolve, resolve));
         let addClubs = () => new Promise((resolve, reject) => xyz.util.performTask(db.addClub, clubs, false).then(resolve, resolve));
   ...
   function extractWorkbookRows(sheet, headers) {
      ...
      let rows = xyz.util.unique(Object.keys(sheet).filter(f => getRow(f) > 1).map(m=>getRow(m)));
   ...
   function identifyWorkbook(workbook) {
      ...
      if (xyz.util.intersection(sheets, ['CourtHive', 'Players', 'Clubs']).length == 2) return 'courthive_imports';
      return 'tournament';
   }
      ...
      if (load.loaded.matches.length) {
         ...
         [].concat(...load.loaded.matches.map(match=>match.players)).forEach(player => {
            if (phash.indexOf(player.hash) < 0) {
               ...
               all_players.push({ 
                  hash: player.hash,
                  first_name: xyz.util.normalizeName(player.first_name, false),
                  last_name: xyz.util.normalizeName(player.last_name, false),
                  club: player.club,
                  ioc: player.ioc,
               });
            }
