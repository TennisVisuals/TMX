...
let player = function() {
      ...
      return new Promise((resolve, reject) => {
         ...
         function displayPoints(player, club, points, ranking_date=new Date()) {
            ...
            Object.keys(cpts).forEach(category => {
               if (xyz.util.isMember(eligible_categories, category) && cpts[category].length) {
                  let tab = category;
            ...
            let dt = (evt) => tournaments.displayTournament({tuid: xyz.util.getParent(evt.target, 'point_click').getAttribute('tuid')});
         ...
         function displayMatches(matches) {
            ...
            xyz.util.addEventToClass('tournament_click', tournamentClick, container.matches.element);
   ...
   function scheduledMatchDetails(match) {
      ...
      genders = xyz.util.unique(genders);
   ...
   function createNewPlayer({player_data={}, category, callback, date=new Date()} = {}) {
      ...
      player_container.birth.element.style.background = xyz.util.validDate(player.birth, daterange) ? 'white' : 'yellow';
      ...
      let saveNewPlayer = () => { 
         let valid_date = xyz.util.validDate(player.birth, daterange);
         if (!valid_date || !player.first_name || !player.last_name || !player.ioc) return;
         player.full_name = `${player.last_name.toUpperCase()}, ${xyz.util.normalizeName(player.first_name, false)}`;
      ...
      function validateBirth(elem, evt) {
         ...
         let valid_date = xyz.util.validDate(datestring, daterange);
