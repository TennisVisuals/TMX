let staging = function() {
   ...
   function processDirective(data) {
      ...
      if (json_data && json_data.directive) {
         ...
         if (json_data.directive == 'load data' && json_data.content) { load.loadJSON(json_data.content); }
   ...
   function receiveTournament({ record, authorized }) {
      ...
      function saveReceivedTournament() {
         ...
         db.addTournament(published_tournament).then(displayTournament, xyz.util.logError);
      }
   ...
   fx.broadcastScore = (match) => {
      ...
      let playerName = (player) => 
            ({ 
               lname: xyz.util.normalizeName(player.last_name, false),
               fname: xyz.util.normalizeName(player.first_name, false),
               name: xyz.util.normalizeName(`${player.first_name} ${player.last_name}`, false),
               ioc: player.ioc,
            });
   ...
   function mergeReceivedEvent() {
      ...
      db.findTournament(revt.tournament.tuid).then(trny => found(trny, authorized), xyz.util.logError);
