let messaging = function() {
   ...
   function fetchTournament(merge_with_tuid, coords, modifyTournament) {
      db.findSetting('fetchTournament').then(s => {
         if (s) {
            let fetchFx = s.fx ? xyz.util.createFx(s.fx) : undefined;
      ...
      function completeFetch(fetched) {
         ...
         if (merge_with_tuid) {
            ...
            db.findTournament(merge_with_tuid).then(existing => mergeTournaments(existing, fetched), xyz.util.logError);
         } else {
      ...
      return new Promise((resolve, reject) => {
         ...
         function normalizePlayers(players) {
            players.forEach(player => {
               ...
               player.last_name = xyz.util.normalizeName(player.last_name, false).trim();
               player.first_name = xyz.util.normalizeName(player.first_name, false).trim();
   ...
   function fetchRankLists(categories) {
      return new Promise((resolve, reject) => {
         ...
         function rankErr(err) {
            ...
            if (messaging.errors) gen.popUpMessage(message);
            reject();
         }
