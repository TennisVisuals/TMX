...
var tournamentParser = function() {
   ...
   tp.drawPlayers = ({sheet}) => {
      ...
      function extractPlayer(row, draw_position) {
         ...
         player.hash = xyz.util.nameHash(player.first_name + player.last_name);
   ...
   tp.drawPosition = ({full_name, players, idx = 0}) => {
      ...
      let tournament_player = players.filter(player => player.full_name && xyz.util.normalizeName(player.full_name) == xyz.util.normalizeName(full_name))[idx];
   ...
   let columnMatches = (sheet, round, players) => {
      ...
      round.column_references.forEach((reference, index) => {
         ...
         if (draw_position != undefined) {
            ...
            matches.push({ winners, result: xyz.util.normalizeScore(cell_value) });
            winners = [];
         }
      });
   ...
   let findEmbeddedRounds = (rounds) => {
      ...
      rounds.forEach((round, index) => {
         ...
         if (embedded) {
            ...
            let indices = xyz.util.numArr(round.matches.length);
      ...
      if (round_robin) {
         ...
         player_rows.forEach((player_row, player_index) => {
            ...
            player_result_referencess.forEach(reference => {
               ...
               let result = xyz.util.normalizeScore(tp.value(sheet[reference]));
            ...
            if (target && target.match(/\d+/)) {
               ...
               if (result) {
                  let match = {
                     ...
                     loser_names: [loser],
                     round: 'RRF',
                     result: xyz.util.normalizeScore(result),
                     gender: gender,
                  }
                  matches.push(match);
               }
   ...
   tp.drawResults = (workbook, tuid) => {
      ...
      let processSheet = ({ sheet_names, sheet_name }) => {
         ...
         let processMatch = (match) => {
            ...
            let puids = players.map(player => xyz.util.nameHash(player.first_name + player.last_name));
