...
let config = function() {
   ...
   function idiomLimit(opts) {
      ...
      gen.escapeModal();
      xyz.util.addEventToClass('flag_opt', selectIOC);
      function selectIOC(evt) {
         let elem = xyz.util.findUpClass(evt.target, 'flag_opt');
   ...
   function idiomSelector() {
      return new Promise((resolve, reject) => {
         ...
         db.findAllIdioms().then(prepareIdioms, xyz.util.logError);

         function prepareIdioms(idioms) {
            idioms.forEach(lang.define);
            db.findSetting('defaultIdiom').then(setupIdioms, xyz.util.logError);
         }
      });
   }
   ...
   function editSettings() {
      ...
      function displaySettings(settings) {
         ...
         if (v.draws) {
            ...
            container.fixed_bye_order.element.checked = xyz.util.string2boolean(env.drawFx.fixed_bye_order);
            ...
            container.auto_byes.element.checked = xyz.util.string2boolean(env.drawFx.auto_byes);
            ...
            container.compressed_draw_formats.element.checked = xyz.util.string2boolean(env.drawFx.compressed_draw_formats);
            ...
            container.display_flags.element.checked = xyz.util.string2boolean(env.draws.tree_draw.flags.display);
            ...
            container.after_matches.element.checked = xyz.util.string2boolean(env.draws.tree_draw.schedule.after);
            ...
            container.court_detail.element.checked = xyz.util.string2boolean(env.draws.tree_draw.schedule.courts);
         ...
         if (v.publishing) {
            ...
            container.require_confirmation.element.checked = xyz.util.string2boolean(env.publishing.require_confirmation);
            ...
            container.publish_on_score_entry.element.checked = xyz.util.string2boolean(env.publishing.publish_on_score_entry);
   ...
   function rankCalc(selected_date) {
      ...
      function dpp(evt) {
         var elem = xyz.util.getParent(evt.target, 'player_rank');
      ...
      rank.calculateRankLists(week, year).then(categories => { 
         ...
         if (Object.keys(categories).length) {
            ...
            xyz.util.addEventToClass('print', pdfList, container.container.element)
            xyz.util.addEventToClass('category_csv', exportCategorySpreadsheet, container.container.element)
            xyz.util.addEventToClass('spreadsheet', exportCategoriesSpreadsheet, container.container.element)
            xyz.util.addEventToClass('icon_json', exportJSON, container.container.element)
      ...
      searchBox.populateSearch.players = function({filtered} = {}) {
         ...
         db.findAllPlayers().then(arr => {
            ...
            let lastfirst = arr.map(player => { 
               let label = `${xyz.util.normalizeName(player.last_name).toUpperCase()} ${xyz.util.normalizeName(player.first_name)}`;
               if (player.birth) label += ` [${new Date(player.birth).getFullYear()}]`;
               return { value: player.puid, label, }
            });
      ...
      if (o.components.tournament_search) searchBox.populateSearch.tournaments = function() {
         db.findAllTournaments().then(arr => {
            ...
            searchBox.typeAhead.list = !arr.length ? [] : arr.map(tournament => { 
               ...
               let start_date = xyz.util.formatDate(new Date(tournament.start));
               let label = xyz.util.normalizeName(`${category} ${tournament.name} [${start_date}]`);
   ...
   function receiveSettings(data) {
      ...
      function update() { updateSettings(data.content).then(()=>envSettings().then(setIdiom, xyz.util.logError), xyz.util.logError); }
      function setIdiom() { db.findSetting('defaultIdiom').then(checkIdiom, xyz.util.logError); }
      ...
      return new Promise((resolve, reject) => {
         ...
         function setEnv(settings) {
            ...
            if (app && app.components) {
               xyz.util.boolAttrs(app.components);
               xyz.util.keyWalk(app.components, o.components);
            ...
            if (misc && misc.settings) {
               xyz.util.boolAttrs(misc.settings);
               xyz.util.keyWalk(misc.settings, env);
            }

            let draws = getKey('drawSettings');
            if (draws && draws.settings) {
               xyz.util.boolAttrs(draws.settings);
               xyz.util.keyWalk(draws.settings, env.draws);
            }

            // TODO: all keys need to replace 'treeDraw' with 'drawSettings'
            let td = getKey('treeDraw');
            if (td) {
               xyz.util.boolAttrs(td.options);
            ...
            if (settings_tabs && settings_tabs.settings) {
               xyz.util.boolAttrs(settings_tabs.settings);
               xyz.util.keyWalk(settings_tabs.settings, o.settings_tabs);
            }

            let publishing = getKey('publishingSettings');
            if (publishing && publishing.settings) {
               xyz.util.boolAttrs(publishing.settings);
               xyz.util.keyWalk(publishing.settings, env.publishing);
            }

            let default_score_format = getKey('defaultScoreFormat');
            if (default_score_format && default_score_format.settings) {
               xyz.util.boolAttrs(default_score_format.settings);
               xyz.util.keyWalk(default_score_format.settings, env.default_score_format);
            }

            let draw_fx = getKey('drawFx');
            if (draw_fx && draw_fx.settings) {
               xyz.util.boolAttrs(draw_fx.settings);
               xyz.util.keyWalk(draw_fx.settings, env.drawFx);
            }

            let rd = getKey('rrDraw');
            if (rd) {
               xyz.util.boolAttrs(rd.options);
   ...
   fx.pointsTable = ({ calc_date }) => {
      ...
      if (!org_tables || !calc_date || !org_tables.validity) {
         ...
         let calc_date_string = xyz.util.formatDate(calc_date);
   ...
   fx.eligibleCategories = ({age, calc_date}) => {
      ...
      let categories = Object.keys(points_table.categories)
         .filter(category => {
            ...
            let valid = xyz.util.range(from, to+1).indexOf(age) >= 0;
   ...
   function updateTournaments({ merge }={}) {
      ...
      let addNew = (trnys) => xyz.util.performTask(db.addTournament, trnys, false).then(done, done);
      let mergeTournaments = (trnys) => xyz.util.performTask(mergeTournament, trnys, false).then(done, done);
      let notConfigured = (err) => { done(); gen.popUpMessage((err && err.error) || lang.tr('phrases.notconfigured')); }
      if (merge) {
      ...
      function mergeTournament(trny) {
         return new Promise((resolve, reject) => {
            db.findTournament(trny.tuid).then(mergeExisting, xyz.util.logError);
   ...
   function updateClubs() {
      ...
      let done = () => gen.busy.done(id);
      let addNew = (clubs) => xyz.util.performTask(db.addClub, clubs, false).then(done, done);
      let notConfigured = (err) => { done(); gen.popUpMessage((err && err.error) || lang.tr('phrases.notconfigured')); }
   ...
   function refreshAction() {
      ...
      if (searchBox.category == 'tournaments') {
         ...
         gen.okCancelMessage(message, mergeList, () => gen.closeModal());
...
//         players: gen.exportRange({ label: lang.tr('bd'), id_names: { start: 'py_start', end: 'py_end', export: 'py_export' }}),
   ...
   function displayClubs() {
      let cc = (evt) => displayClub(xyz.util.getParent(evt.target, 'club_click').getAttribute('cuid'));
