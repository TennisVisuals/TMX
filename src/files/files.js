!function() {
   ...
   let normalID = (id) => xyz.util.replaceDiacritics(id).toUpperCase();
   ...
   exp.matchRecord = (match) => {
      ...
      return {
         ...
         "Winner 1 Name": xyz.util.normalizeName(`${players[winners[0]].last_name}, ${players[winners[0]].first_name}`),
         ...
         "Winner 1 City": xyz.util.replaceDiacritics(players[winners[0]].city || ''),
         "Winner 1 State": '',
         "Winner 1 Country": players[winners[0]].ioc || '',
         "Winner 1 College": '',
         "Winner 2 Name": dbls ? xyz.util.normalizeName(`${players[winners[1]].last_name}, ${players[winners[1]].first_name}`) : '',
         ...
         "Winner 2 City": xyz.util.replaceDiacritics(dbls ? (players[winners[0]].city || '') : ''),
         "Winner 2 State": '',
         "Winner 2 Country": dbls ? (players[winners[1]].ioc || '') : '',
         "Winner 2 College": '',

         "Loser 1 Name": xyz.util.normalizeName(`${players[losers[0]].last_name}, ${players[losers[0]].first_name}`),
         ...
         "Loser 1 City": xyz.util.replaceDiacritics(players[losers[0]].city || ''),
         "Loser 1 State": '',
         "Loser 1 Country": players[losers[0]].ioc || '',
         "Loser 1 College": '',
         "Loser 2 Name": dbls ? xyz.util.normalizeName(`${players[losers[1]].last_name}, ${players[losers[1]].first_name}`) : '',
         ...
         "Loser 2 City": xyz.util.replaceDiacritics(dbls ? (players[losers[0]].city || '') : ''),
         "Loser 2 State": '',
         ...
         "Draw Team Type": xyz.util.normalizeName(match.format) || '',
   ...
   exp.downloadRankings = (rankings) => {
      if (!rankings) {
         ...
         db.findAllClubs().then(processCategories, xyz.util.logError);
   ...
   function teamName(match, team) {
      if (team.length == 1) {
         ...
         let full_name = `${xyz.util.normalizeName(p.first_name, false)} ${xyz.util.normalizeName(p.last_name, false).toUpperCase()}`; 
         return `${full_name}${club}`;
      } else {
         return team.map(p => xyz.util.normalizeName(match.players[p].last_name, false).toUpperCase()).join('/');
      }
   }
   ...
   function scheduleCell(match, lines=false) {
      ...
      function unknownBlock(match, pindex) {
         ...
         function potentialBlock(p) { return xyz.util.normalizeName(p.last_name, false).toUpperCase(); }
      }
   ...
   function schedulePDF(tournament, day, courts, matches, logo) {
      ...
      let rounds = xyz.util.unique(matches.map(m=>parseInt(m.schedule.oop_round)));
      let max_round = Math.max(minimum_rows, ...rounds);
      let row_matches = xyz.util.range(1, max_round + 1).map(oop_round => matches.filter(m=>m.schedule.oop_round == oop_round));
      ...
      var draw_sheet = {
         ...
         table: {
            ...
            body: [
               ...
               [ 
                  { text: xyz.util.formatDate(new Date(tournament.start)), style: 'tableData' },
   ...
   function drawSheet({ tournament={}, images, logo, selected_event, event, info }) {
      ...
      let rank_list_date = xyz.util.formatDate(`${year}-${month+1}-01`);
   ...
   function doublesSignInSheet({ tournament={}, teams=[], players=[], category, gender, event_name='', logo, doc_name }) {
      ...
      let date = xyz.util.formatDate(tournament.start);
      let tournament_id = tournament.display_id || (tournament.tuid.length < 15 ? tournament.tuid : '');
      ...
      rows = rows.map((row, i) => {
         if (row) {
            return [
               ...
               { text: xyz.util.normalizeName(row[0].first_name, false) },
               ...
               { text: xyz.util.normalizeName(row[1].first_name, false) },
      ...
      rows = rows.map((row, i) => {
         if (row) {
            return [
               ...
               { text: xyz.util.normalizeName(row.first_name, false) },
