...
let scoreBoard = function() {
   ...
   function setMatchScore({ round, container, teams, existing_scores, score_format={}, callback }) {
      ...
      let flags = true;
      let sobj = xyz.displayFx.scoreBoard({ container, teams, flags });
      ...
      let displayActions = (bool) => {
         ...
         xyz.displayFx.toggleVisible({ elem: sobj.actions.element, type: 'edit', height: 50, visible });
      ...
      function configureActionButtons() {
         ...
         function toggleScoring() {
            ...
            let duration = 800;
            xyz.displayFx.toggleVisible({ elem: sobj.edit_scoring.element, type: 'edit', height: 100, visible, duration });
            if (visible) {
      ...
      function configureScoreSelectors() {
         ...
         xyz.util.range(0, upper_range).forEach(n => options.push({ key: n, value: n }));

         let scoreChange1 = (value) => scoreChange(0, value);
         let scoreChange2 = (value) => scoreChange(1, value);
         xyz.dd.attachDropDown({ id: sobj.p1selector.id, options, border: false });
         xyz.dd.attachDropDown({ id: sobj.p2selector.id, options, border: false });
         sobj.p1selector.ddlb = new xyz.dd.DropDown({ element: sobj.p1selector.element, onChange: scoreChange1, id: sobj.p1selector.id });
         sobj.p2selector.ddlb = new xyz.dd.DropDown({ element: sobj.p2selector.element, onChange: scoreChange2, id: sobj.p2selector.id });
      ...
      function configureOutcomeSelectors() {
         ...
         xyz.dd.attachDropDown({ id: sobj.p1action.id, options: actions, border: false });
         xyz.dd.attachDropDown({ id: sobj.p2action.id, options: actions, border: false });
         sobj.p1action.ddlb = new xyz.dd.DropDown({ element: sobj.p1action.element, onChange: outcomeChange1, id: sobj.p1action.id });
         sobj.p2action.ddlb = new xyz.dd.DropDown({ element: sobj.p2action.element, onChange: outcomeChange2, id: sobj.p2action.id });
      ...
      function setScoreDisplay({ selected_set, tiebreak }) {
         ...
         let p1scores = before.map((s, i) => xyz.displayFx.setScore({ setnum: i, score: s[0] })).join('');
         let p2scores = before.map((s, i) => xyz.displayFx.setScore({ setnum: i, score: s[1] })).join('');
         ...
         p1scores = after.map((s, i) => xyz.displayFx.setScore({ setnum: +selected_set + 1 + i, score: s[0] })).join('');
         p2scores = after.map((s, i) => xyz.displayFx.setScore({ setnum: +selected_set + 1 + i, score: s[1] })).join('');
         sobj.p1scores_e.element.innerHTML = p1scores;
         sobj.p2scores_e.element.innerHTML = p2scores;

         let ss = (evt) => setClick(xyz.util.getParent(evt.target, 'set_number').getAttribute('setnum'));
         xyz.util.addEventToClass('set_number', ss, sobj.scoreboard.element)
      }
   ...
   function configureScoring(sobj, f, changeFx) {
      ...
      sobj.bestof.ddlb = new xyz.dd.DropDown({ element: sobj.bestof.element, id: sobj.bestof.id, onChange: setBestOf });
      ...
      sobj.setsto.ddlb = new xyz.dd.DropDown({ element: sobj.setsto.element, id: sobj.setsto.id, onChange: setsTo });
      ...
      sobj.tiebreaksat.ddlb = new xyz.dd.DropDown({ element: sobj.tiebreaksat.element, id: sobj.tiebreaksat.id, onChange: setTiebreakAt });
      ...
      sobj.tiebreaksto.ddlb = new xyz.dd.DropDown({ element: sobj.tiebreaksto.element, id: sobj.tiebreaksto.id });
      ...
      sobj.finalset.ddlb = new xyz.dd.DropDown({ element: sobj.finalset.element, id: sobj.finalset.id, onChange: finalSet });
      ...
      sobj.supertiebreakto.ddlb = new xyz.dd.DropDown({ element: sobj.supertiebreakto.element, id: sobj.supertiebreakto.id, onChange: superTiebreakTo });
