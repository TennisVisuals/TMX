...
function treeDraw() {
   ...
   function chart(opts) {
      ...
      let calcRoundWidth = () => {
         ...
         invert_first = !o.draw.feed_in && (o.invert_first || round_width < o.invert_threshold) ? true : false;
      ...
      let flags = round_width < o.flags.threshold ? false : o.flags.display;
      ...
      let nodes = tree(hierarchy).descendants();
...
function drawFx(opts) {
   ...
   function treeInfo(draw) {
      ...
      let hierarchy = d3.hierarchy(draw);
      let nodes = calc_tree(hierarchy).descendants();
   ...
   */
   ...
   function distributeByes({ draw, num_players }) {
      ...
      if (!info.structural_byes.length) {
         ...
         pairs_no_seed_or_bye = pairs_no_seed.filter(pair => !intersection(pair, assignment).length);
   ...
   function matchesPlayers(matches, key='puid') { 
      ...
      // let players = Object.assign(...[].concat(...matches.map(m=>m.players)).map(m=>{ return { [m[key] || m['hash']]: m }})); 
