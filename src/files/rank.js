!function() {
   ...
   function fullName(player) { return `${player.last_name.toUpperCase()}, ${xyz.util.normalizeName(player.first_name, false)}`; }
   ...
   rank.calcMatchesPoints = ({ matches, points_table, points_date }) => {
      ...
      matches.forEach(match => {
         ...
         let first_round_points = ranking_attributes.first_round_points && xyz.util.string2boolean(ranking_attributes.first_round_points[match.event.draw_type]);
         ...
         Object.keys(categories).forEach(category => {
            ...
            Object.keys(categories[category]).forEach(gender => {
               ...
               categories[category][gender].forEach((player, i) => {
                  ...
                  if (xyz.util.isMember(eligible_categories, ranking_category) && player.points.total) {
   ...
   rank.calcAllPlayerPoints = (start_date, end_date = new Date().getTime()) => {
      ...
      return new Promise((resolve, reject) => {
         ...
         function cP(points) {
            let puids = xyz.util.unique(points.map(p => p.puid));
   ...
   rank.calcPlayerDate = (player, points, ranking_date) => {
      ...
      Object.keys(cpts).map(ctgy => {
         ...
         let reduce = (point_events) => point_events.map(p => xyz.util.numeric(p.points)).reduce((a, b) => a + b, 0);
         ...
         if (xyz.util.isMember(categories, category) && total > 0) rpts[category] = { total, singles, doubles, singles_up, doubles_up, team };
      });
   ...
   rank.calculateRankingPoints = (player, points, ranking_date) => {
      ...
      function pDate(p) { return new Date(xyz.util.formatDate(p)).getTime(); }
      ...
      let cpts = {};
      let categories = xyz.util.uunique([].concat(...valid.map(v=>v.category), ...eligible_categories))
   ...
   rank.eligibleCategories = ({birthday, birth_year, calc_date, calc_year}) => {
      if (birthday && xyz.util.validDate(birthday)) birth_year = new Date(birthday).getFullYear();
      if (calc_date && xyz.util.validDate(calc_date)) calc_year = new Date(calc_date).getFullYear();
   ...
   rank.calcTournamentPoints = (tournament, callback) => {
      db.findTournamentMatches(tournament.tuid).then(processMatches, xyz.util.logError);
