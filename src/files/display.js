!function() {
   ...
   let fullName = (p) => `${p.last_name.toUpperCase()}, ${xyz.util.normalizeName(p.first_name, false)}`;
   ...
   function scheduledSort(scheduled) {
      ...
      // extra xyz.util.unique required here because of leagacy situation where rounds were both ints and strings
      let rounds = xyz.util.unique(scheduleAttributeValues(scheduled, 'oop_round').map(r=>parseInt(r))).sort((a, b) => a - b);
   ...
   function displayDate(timestamp) {
      ...
      return [xyz.util.zeroPad(date.getMonth() + 1), xyz.util.zeroPad(date.getDate())].join('&#8209;');
      // return [date.getFullYear(), xyz.util.zeroPad(date.getMonth() + 1), xyz.util.zeroPad(date.getDate())].join('-');
      // return [xyz.util.zeroPad(date.getDate()), xyz.util.zeroPad(date.getMonth() + 1), date.getFullYear()].join('&nbsp;');
   }
   ...
   gen.markAssigned = (e) => {
      ...
      e.select('.action_player').text(xyz.util.normalizeName([searchBox.active.player.first_name, searchBox.active.player.last_name].join(' '), false));
   ...
   function actionRow(action, index) {
      ...
      let html = `
         <div class='action_edit section_row detail flexrow ${odd ? 'row_odd' : ''}' action_index='${index}'>
            ...
            <div class='action_name'>${xyz.util.normalizeName(player_name, false)}</div>
   ...
   gen.activePlayer = (p, club) => {
      let birthday;
      let name = xyz.util.normalizeName(p.first_name + ' ' + p.last_name, false);
   ...
   function playerPointRow(points, odd, expire_date) {
      ...
      let formatted_date = [xyz.util.zeroPad(date.getMonth() + 1), xyz.util.zeroPad(date.getDate()), date.getFullYear()].join('/');
   ...
   function formatTeams({tournament, match, which, puid}) {
      ...
      function playerBlock(pindex, side) {
         ...
         var first_name = xyz.util.normalizeName(p.first_name, false);
         var last_name = p.last_name ? xyz.util.normalizeName(p.last_name, false).toUpperCase() : '';
      ...
      function potentialBlock(p, side) {
         ...
         var last_name = p.last_name ? xyz.util.normalizeName(p.last_name, false).toUpperCase() : p.qualifier ? 'Qualifier' : '';
         var seed = p.seed ? ` [${p.seed}]` : '';
   ...
   function matchBlock({ tournament, headers=true, title, divider, matches, type, puid }) {
      function matchTime(match) {
         ...
         function duration(start, end) {
            ...
            return `${xyz.util.zeroPad(hours)}:${xyz.util.zeroPad(minutes)}`;
         }
   ...
   gen.createNewTournament = (title, tournament = {}) => {
      ...
      let start = !tournament.start ? '' : xyz.util.formatDate(tournament.start);
      let end =   !tournament.end   ? '' : xyz.util.formatDate(tournament.end); 
      ...
      */
      ...
      container.start.element.style.background = xyz.util.validDate(tournament.start) ? 'white' : 'yellow';
      container.end.element.style.background = xyz.util.validDate(tournament.end) ? 'white' : 'yellow';
   ...
   gen.playerInfo = (p, club) => {
      ...
      var name = xyz.util.normalizeName([p.first_name, p.last_name].join(' '), false);
   ...
   gen.tournamentPenalties = (tournament, penalties, saveFx) => {
      ...
      xyz.util.addEventToClass('player_penalty', removePenalty, id_obj.penalties.element);
      function removePenalty(evt) {
         let element = xyz.util.getParent(evt.target, 'player_penalty');
   ...
   function tpRow(tournament, p, i, j, gender, additional_attributes) {
      ...
      let ranking = xyz.util.numeric(p.category_ranking) || '';
   ...
   gen.identifyPlayer = (p) => {
      ...
      let html = `
         ...
         <div class='pdata flexcol flexcenter'>
            <div class='pdata_row'>
               <div class='pdata_label'>${lang.tr('fnm')}:</div><div class="data_value">${xyz.util.normalizeName(p.first_name, false)}</div>
            </div>
            <div class='pdata_row'>
               <div class='pdata_label'>${lang.tr('lnm')}:</div><div class="data_value">${xyz.util.normalizeName(p.last_name, false)}</div>
            </div>
            <div class='pdata_row'>
   ...
   gen.scaleTeams = (container) => {
      ...
      Array.from(scheduled_teams).forEach(el => xyz.util.scaleFont(el));
   }
   ...
   function scheduledMatchHTML(match) {
      ...
      function potentialBlock(p) {
         let last_name = p.last_name ? xyz.util.normalizeName(p.last_name, false).toUpperCase() : p.qualifier ? 'Qualifier' : '';
   ...
   gen.umpirePicker = ({ tournament, callback }) => {
      ...
      function pickerHTML({ tournament }) {
         ...
         let umpire_list = umpires.map(h=>`<li class='hour'>${xyz.util.zeroPad(h)}</li>`).join('');
         let html = `
   ...
   gen.timePicker = ({ value, time_string, hour_range, minute_increment, minutes, callback }) => {
      ...
      xyz.util.addEventToClass('hour', setHour);
      xyz.util.addEventToClass('minute', setMinute);
      ...
      function setTime() { time.value = `${parseInt(hour)}:${xyz.util.zeroPad(minute)}`; }
      function returnValue() { 
         if (typeof callback == 'function') callback(time.value);
         document.body.style.overflow  = null;
         backplane.remove(); 
      ...
      function pickerHTML({ hour, minute, hour_range = {}, minute_increment, minutes } = {}) {
         let hour_list = xyz.util.range(hour_range.start || 0, (hour_range.end || 23) + 1)
            .map(h=>`<li class='hour'>${xyz.util.zeroPad(h)}</li>`).join('');
         let minute_end = Math.abs(60 / (minute_increment || 1));
         minutes = minutes || xyz.util.range(0, minute_end).map((m, i)=>i*(minute_increment || 1));
         let minute_list = minutes
            .map(m=>`<li class='minute'>${xyz.util.zeroPad(m)}</li>`).join('');
         let html = `
               <span class="time-picker">
   ...
   gen.dateSelector = ({ date, date_element, dateFx, container }) => {
      ...
      var datePicker = new Pikaday({
         ...
         onSelect: function() { 
            ...
            date = new Date(xyz.util.dateUTC(this_date));
