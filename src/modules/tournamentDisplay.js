...
import { scheduleFx as sfx } from './scheduleFx';
import { rrDraw, treeDraw, drawFx } from './drawFx';
import { tournamentFx as tfx } from './tournamentFx';
...
export const tournamentDisplay = function() {
   ...
   function displayTournament({tuid, selected_tab, editing} = {}) {
      ...
      function go(tournament, dbmatches) {
         ...
         createTournamentContainer({tournament, dbmatches, selected_tab, editing});
      }
   }

   fx.createTournamentContainer = createTournamentContainer;
   function createTournamentContainer({tournament, dbmatches, selected_tab, editing}) {
      ...
      legacyProcess.tournamentGenders(tournament, dbmatches);
      ...
      if (tfx.sameOrg(tournament) && !tournament.delegated) {
         displayGen.enterFx = () => {
            if (!state.edit) { activateEdit(); }
            displayGen.enterFx = undefined;
         };
      }
      ...
      domFx.addEventToClass(classes.auto_draw, toggleAutoDraw);
      domFx.addEventToClass(classes.gem_seeding, toggleGemSeeding);
      domFx.addEventToClass(classes.ratings_filter, toggleRatingsFilter);

      attachFilterToggles(classes, updateFilters);
      domFx.addEventToClass(classes.ranking_order, () => enableManualRankings());
      domFx.addEventToClass(classes.reg_link, () => editRegistrationLink());
      domFx.addEventToClass(classes.refresh_registrations, (evt) => {
         if (evt.ctrlKey || evt.shiftKey) return replaceRegisteredPlayers(true);
         updateRegisteredPlayers(true);
      });
      domFx.addEventToClass(classes.refresh_registrations, () => replaceRegisteredPlayers(true), undefined, 'contextmenu');

      // set up printing events
      domFx.addEventToClass(classes.print_sign_in, playersTabPDFs);
      domFx.addEventToClass(classes.print_draw, printDraw);
      domFx.addEventToClass(classes.print_draw, () => console.log('context menu print'), document, 'contextmenu');

      domFx.addEventToClass(classes.print_schedule, printSchedule);
      domFx.addEventToClass(classes.schedule_matches, scheduleMatches);
      function scheduleMatches() {
         ...
         if (scheduling_active) {
            ...
            let search_box = search_field && domFx.getParent(search_field, 'schedule_box');
            if (search_box) search_box.innerHTML = sfx.opponentSearch();
         }
      ...
      domFx.addEventToClass(classes.schedule_details, scheduleDetails);
      ...
      domFx.addEventToClass(classes.publish_players, publishPlayers);
      ...
      function publishPlayers() {
         ...
         let publist = displayGen.publishPlayerList(tournament.publishing.players);
         publist.publish.element.addEventListener('click', configurePlayerPublishing);
         publist.unpublish.element.addEventListener('click', disablePlayerPublishing);
         ...
         function configurePlayerPublishing() {
            tournament.publishing.players = {
               ...
               sex: publist.sex.element.checked,
               ioc: publist.ioc.element.checked && env.assets.flags,
               sort: publist.sort.ddlb.getValue(),
               up_to_date: true
            };
            displayGen.closeModal();
            publishTournamentInfo(tournament, true);
            playersPublishState();
         }
         
         function disablePlayerPublishing() {
            delete tournament.publishing.players;
            displayGen.closeModal();
            if (tournament.infoPublished) publishTournamentInfo(tournament, true);
            playersPublishState();
         }
      }
      ...
      container.delegate.element.addEventListener('click', () => {
         ...
         function revoke() {
            coms.revoked = (result) => {
               if (!result.revoked) { displayGen.okCancelMessage(lang.tr('tournaments.noauth'), cleanUp); }
            ...
            coms.delegated = (result) => {
               if (!result.keyset) {
                  ...
                  let ctext = `
                     ...
                     <canvas id='qr'></canvas>
                     `;
                  displayGen.okCancelMessage(ctext, cleanUp);
                  genQUR(message);
               }
      ...
      function tournamentLinkAuth() {
         modalViews.closeModal();
         let url = `${location.origin}/Live/${tournament.org.abbr}`;
         window.open(url, '_blank');
      }
      function tournamentLinkNoAuth() {
         modalViews.closeModal();
         let url = `${location.origin}/Live/${tournament.org.abbr}?test`;
         window.open(url, '_blank');
      }

      function downloadQRcode() { displayQRdialogue(tournament); }
      function displayQRdialogue(tournament) {
         modalViews.closeModal();
         ...
         let ctext = `
            ...
            `;
         displayGen.okCancelMessage(ctext, () => displayGen.closeModal());
         genQUR(message);
      ...
      function publishTournamentInfo(tournament, republish) {
         tournament.org = env.org;
         if (!republish && tournament.infoPublished) return unpublishTournamentInfo(tournament);
      ...
      displayGen.pubStateTrnyInfo(container.pubStateTrnyInfo.element, tournament.infoPublished);
      displayGen.tournamentPublishState(container.push2cloud_state.element, tournament.pushed2cloud);
      if (container.localdownload_state && container.localdownload_state.element) {
...
//              'table', 'tbody', 'tr', 'td', 'th', 'thead'
      ...
      if (container.localdownload && container.localdownload.element) {
      ...
      container.player_reps.element.addEventListener('click', () => {
         ...
         if (displayed.draw_event) {
            ...
            let valid_reps = tournament.players
               ...
               .map(p=>{
                  let player = stringFx.normalizeName(`${p.first_name} ${p.last_name}`);
            ...
            modal.player_rep1.element.addEventListener("awesomplete-selectcomplete", () => { rep1_selection_flag = true; }, false);
            modal.player_rep1.element.addEventListener('keydown', catchTab , false);
            modal.player_rep1.element.addEventListener("keyup", e => { 
            ...
            modal.player_rep2.element.addEventListener("awesomplete-selectcomplete", () => { rep2_selection_flag = true; }, false);
            modal.player_rep2.element.addEventListener('keydown', catchTab , false);
            modal.player_rep2.element.addEventListener("keyup", e => { 
               // auto select first item on 'Enter' *only* if selectcomplete hasn't been triggered
      ...
      container.autoschedule.element.addEventListener('contextmenu', autoSchedule);
      ...
      function cMenu({ selector, coords, options, clickAction }) {
         ...
         if (options.length) {
            ...
            setTimeout(function() { setup(x, y); }, 200);
         }
      ...
      function autoSchedule() {
         ...
         let pending_by_format = pending_matches.sort(a => a.format == 'singles' ? 0 : 1);

         let upcoming_by_format = upcoming_matches.sort(a => a.format == 'singles' ? 0 : 1);
         ...
         if (match_round_names.length > 1) {
            ...
            let getRound = () => config.at_row.ddlb.getValue();
            let getCourt = () => config.column.ddlb.getValue();
            config.order.element.addEventListener('click', () => { order_priority = true; displayGen.closeModal(); doIt(getRound(), getCourt()); });
            config.round.element.addEventListener('click', () => { displayGen.closeModal(); doIt(getRound(), getCourt()); });
            ...
            // filter courts so that unavilable courts are not included
            let available_courts = util.unique(available_cells.map(c=>c.court));
            let court_options = courts
               .filter(c=>available_courts.indexOf(c.name) >= 0)
               .map(c=>({ key: c.name, value: c.name }));
            court_options.unshift({ key: '-', value: '' });
            config.column.ddlb.setOptions(court_options);
            config.column.ddlb.setValue(court_options[0].value, 'white');
         } else {
            doIt();
         }

         function doIt(at_row=1, court) {
            available_cells = available_cells.filter(cell => cell.oop_round >= at_row);
            if (court) { available_cells = available_cells.filter(cell => cell.court == court); }

            let to_be_scheduled = sfx.sortedUnscheduled(tournament, filtered_unscheduled, order_priority);
      ...
      function checkConflicts(day_matches) {
         ...
         let issues = sfx.schedulingIssues(day_matches);
         ...
         Object.keys(status_map).forEach(muid => sfx.colorCell(container.schedule.element, muid, status_map[muid]));
      ...
      function enableAddPlayer() {
         ...
         let calc_date = dateFx.offsetDate(tournament_date);
      ...
      */
      ...
      function activateEdit() {
         ...
         domFx.moveNode('content', container.container.id);
      ...
      function deactivateEdit() {
         ...
         saveTournament(tournament);
         setEditState();
         if (env.actions.finish2calendar) calendarFx.displayCalendar();
      }
      ...
      /*
      // TODO: hook this up to icon for sharing team
      ...
      */
      ...
      function delegateMatch(match, teams, score_format) {
         let evt = tfx.findEventByID(tournament, match.euid || (match.event && match.event.euid));
         if (!evt) { return console.log('error: no event'); }
         let key_uuid = (+new Date).toString(36).slice(-6).toUpperCase();
      ...
      function editAction() {
         ...
         container.cloudfetch.element.addEventListener('click', displayGen.cloudFetchMenu);
      }

      function mergeEvents() { modalViews.closeModal(); coms.requestTournamentEvents(tournament.tuid); }
      ...
      eventManager
         ...
         .register('generateKey', 'tap', authorizeUser)
         .register('authorizeAdmin', 'tap', authorizeAdmin)
         .register('revokeAuthorization', 'tap', revokeAuthorization)
         .register('downloadQRcode', 'tap', downloadQRcode)
         .register('tournamentLinkAuth', 'tap', tournamentLinkAuth)
         .register('tournamentLinkNoAuth', 'tap', tournamentLinkNoAuth)
         .register('tournamentLinks', 'tap', displayGen.tournamentLinks)
         .register('googleTournamentLocation', 'tap', googleTournamentLocation)
         .register('currentTournamentLocation', 'tap', currentTournamentLocation)
         .register('singlesSignInPDF', 'tap', singlesSignInPDF)
         .register('doublesSignInPDF', 'tap', doublesSignInPDF)
         .register('playersListPDF', 'tap', playersListPDF)
         .register('mergeEvents', 'tap', mergeEvents)
         .register('fetchTournamentRecord', 'tap', fetchTournamentRecord);

      function authorizeActions() {
         if (state.admin) {
            displayGen.authorizeActions();
         } else {
            console.log('offer to enter key');
         }
      }
      ...
      function authorizeAdmin() {
         ...
         function submitKey() {
            ...
            eventManager.register('tournamentAuthorization', 'tap', receiveAuth);
            displayGen.tournamentPublishState(container.push2cloud_state.element, tournament.pushed2cloud);
         }
      ...
      function googleTournamentLocation() { geoFx.getLatLng({ successFx: setTournamentLatLng }); }
      function currentTournamentLocation() { geoFx.getUserPosition({ successFx: setTournamentLatLng }); }

      function setTournamentLatLng({ latitude, longitude }) {
         if (!tournament.address) tournament.address = {};
         tournament.address.latitude = latitude;
         tournament.address.longitude = longitude;
         container.latitude.element.value = latitude;
         container.longitude.element.value = longitude;
         saveTournament(tournament);
      }
      ...
      function setEditState() {
         ...
         container.x.element.style.display = state.edit ? 'none' : 'inline';
         ...
         if (env.documentation.hoverhelp) {
            document.querySelector('.' + classes.refresh_registrations).classList[state.edit ? 'add' : 'remove'](displayGen.info || 'stub');
         }
         ...
         domFx.eachElementClass(container.team_details.element, 'team_attr_edit', (el) => el.disabled = !state.edit);
         domFx.eachElementClass(container.team_details.element, 'manualorder', (el) => {
            el.disabled = true;
         ...
         let view_icon = state.edit && env.documentation.links ? 'flex' : 'none';
         domFx.eachElementClass(document, 'tiny_docs_icon', (el) => el.style.display=view_icon);
         domFx.eachElementClass(document, 'tiny_tour_icon', (el) => el.style.display=view_icon);
         domFx.eachElementClass(document, 'hints_icon', (el) => el.style.display=view_icon);
         ...
         if (state.edit) {
            db.findSetting('superUser').then(setting => {
               if (setting && setting.auth) {
                  if (stringFx.string2boolean(setting.auth.tournaments)) { enableDownloads(); }
                  if (stringFx.string2boolean(setting.auth.authorize)) { state.admin = true; }
               }
            });
      ...
      function enableTournamentOptions() {
         ...
         [ 'start_date', 'end_date', 'organization', 'organizers', 'location', 'judge', 'latitude', 'longitude' ]
            .forEach(field=>container[field].element.disabled = !bool);
      ...
      function addRegistered(registered_players) {
         registered_players = registered_players.filter(r=>!r.allowed || (r.allowed != 0 && r.allowed != false));
      ...
      // PLAYERS TAB PRINTING
      function playersTabPDFs() {
         if (!tournament.players || !tournament.players.length) return;
         displayGen.playersTabPDFs();
      }
      
      function singlesSignInPDF() {
         modalViews.closeModal();
      ...
      function approvedByRank(e) {
         ...
         if (e.format == 'S') {
            ...
            let isApproved = (p) => e.approved.indexOf(p.id) >= 0;
            let isWildcard = (p) => e.wildcards.indexOf(p.id) >= 0;
            let isLuckyLoser = (p) => e.luckylosers.indexOf(p.id) >= 0;
      ...
      domFx.addEventToClass(classes.team_rankings, () => enableTeamRankings());
      domFx.addEventToClass(classes.roster_link, () => defineRosterLink());
      domFx.addEventToClass(classes.refresh_roster, () => updateRosterPlayers());
      ...
      function updateRosterPlayers(show_notice = true) {
         ...
         fetchFx.fetchGoogleSheet(displayed.team.roster_link).then(done, displayGen.invalidURLorNotShared);
      }
      ...
      function teamList({cleanup} = {}) {
         ...
         if (tournament.teams) {
            if (cleanup) {
               domFx.eachElementClass(container.teams.element, 'teamid', (i) => i.classList.remove('highlight_listitem'));
            ...
            let teams = tournament.teams.map(team => {
            ...
            if (env.documentation.hoverhelp) {
               container.team_details.element.querySelector('.' + classes.team_rankings).classList[opacity ? 'add' : 'remove'](displayGen.infoleft || 'stub');
            }

            let teamDetails = (target) => {
               let clicked_team = domFx.getParent(target, 'teamid');
      ...
      function eventList(regen_drawstab = false) {
         ...
         if (tournament.events && tournament.events.length) {
            events = tournament.events.map(e => {
               ...
               let linked_elimination = tfx.findEventByID(tournament, e.links['E']);
               ...
               let preround_naming = q_subtypes.preround ? lang.tr('draws.preround') : q_subtypes.incidentals ? lang.tr('draws.incidentals') : getKey(draw_types, e.draw_type);
               let draw_type_name = tfx.isPreRound({ env, e }) ? preround_naming : getKey(draw_types, e.draw_type); 
               ...
               let draw_positions = info && info.draw_positions && info.draw_positions.length;
               ...
               if (tournament.teams) {
                  ...
                  opponents = tfx.isConsolationFeedIn(e) ? totalOpponents(linked_elimination) : totalOpponents(e);
               }

               let draw_size = e.structure == 'feed' ? draw_positions : e.draw_size;
         ...
         function totalOpponents(evt) { return !evt ? 0 : (evt.approved.length + (['E', 'S'].indexOf(evt.draw_type) >= 0 ? (evt.qualifiers || 0) : 0)); }
         eventManager.register('displayEventDetails', 'tap', displayEventDetails);

         function displayEventDetails(target) {
            tmxTour.clear();
            let clicked_event = domFx.getParent(target, 'event');
         ...
         domFx.addEventToClass('event', customEventCategory, container.events.element, 'contextmenu');

         domFx.addEventToClass('published_header', unpublishAllEvents, document);
         domFx.addEventToClass('published_header', unpublishAllEvents, document, 'contextmenu');
         domFx.addEventToClass('published_header', (evt) => { if (evt.ctrlKey || evt.shiftKey) return unpublishAllEvents(); });
         if (regen_drawstab) drawsTab();
         enableTournamentOptions();
      }

      function customEventCategory(evt) {
         let clicked_event = domFx.getParent(evt.target, 'event');
         let euid = clicked_event.getAttribute('euid');
      ...
      // function eventPubState(evt) {
      function eventPubState(target) {
      ...
      function addTeam() {
         ...
         added_team.createnew.element.addEventListener('click', createNewTeam);
         ...
         function selectionBox(teams) {
            ...
            teamsearch.addEventListener("awesomplete-selectcomplete", () => { selection_flag = true; teamSelected(this.value); }, false);
      ...
      function newTournamentEvent() {
         tmxTour.clear();
         let genders_signed_in = legacyProcess.tournamentGenders(tournament, dbmatches, (f)=>f.signed_in);
         ...
         if (tfx.isTeam(tournament)) {
            let singles = tfx.getScoreboardSettings({ format: 'singles' });
            let doubles = tfx.getScoreboardSettings({ format: 'doubles' });
            presets = {
               draw_type: 'E',
               format: '',
               category: tournament.category || '',
               ...
               inout: tournament.inout || '',
               scoring_format: { singles, doubles }
            };
         }
      ...
      function displayTeam({ team, index } = {}) {
         ...
         domFx.addEventToClass('manualorder', orderEntry, container.team_details.element, 'keyup');
         domFx.addEventToClass('manualorder', catchTab, container.team_details.element, 'keydown');
         domFx.addEventToClass('team_player', teamPlayerClick, container.team_details.element);
         domFx.addEventToClass('team_player', teamPlayerContext, container.team_details.element, 'contextmenu');
         ...
         function deleteTeam() {
            console.log('delete team... delete players from tournament IF team is sheet sync?');
            closeTeamDetails();
            displayGen.closeModal();
      ...
      function teamPlayerContext(evt) {
         if (state.edit) {
            ...
            let puid = domFx.getParent(evt.target, 'team_player').getAttribute('puid');
            let clicked_player = tournament.players.reduce((p, c) => { if (c.puid == puid) p = c; return p; }, undefined);

            let selectionMade = (choice) => {
      ...
      container.event_edit_name.element.addEventListener('focusout', submitEventName);
      container.event_edit_name.element.addEventListener('keyup', function(k) { if (k.which == 13) { submitEventName(); } });
      ...
      function displayEvent({ e, index } = {}) {
         ...
         function deleteTournamentEvent() {
            ...
            if (tfx.isConsolationFeedIn(linked_consolation)) {
               console.log('delete event: if there is linked consolation delete that event as well?');
            }
            ...
            if (dbmatches) {
               ...
               ({ groups: match_groups, group_draws } = legacyProcess.groupMatches(dbmatches));
            }
      ...
      function determineLinkedDraw(tournament, e, type, linkChanged) {
         ...
         let setLink = (value) => {
            ...
            // if draw is consolation and there is no linnked event then there can be no approved players
            if (e.draw_type == 'C' && !linked_event) {
               e.approved = [];
               delete e.max_round;
            }
      ...
      function configDrawType(e) {
         function linkChanged() {
            if (e.draw_type == 'C') setConsolationConfig();
            eventPlayers(e);
         }
         ...
         if (tfx.isConsolationFeedIn(e)) {
            ...
            let max_feed_rounds = (elimination_info && elimination_info.depth) || 0;
            feed_options = util.range(1, max_feed_rounds).map(v=>({ key: v, value: v}));
         }

         var sequential_options = [1].map(v=>({ key: v, value: v}));
         ...
         function setStructure(value) {
            ...
            if (value == 'feed' && e.draw_type == 'E' && linked_consolation && linked_consolation.structure == 'feed') {
               ...
               linked_consolation.regenerate = 'Linked Elimination Structure Change';
            }

            let linked_elimination = tfx.findEventByID(tournament, e.links['E']);
            if (value == 'feed' && e.draw_type == 'C') {
               removeMaxRound(e);
               e.approved = [];
               if (linked_elimination) {
                  let elimination_info = dfx.drawInfo(linked_elimination.draw);
                  e.feed_base = dfx.calcFeedBase({ draw_positions: elimination_info && elimination_info.draw_positions }) || 0;
                  e.feed_rounds = elimination_info && elimination_info.depth - 1;
               }
            }
            e.structure = value;
            configDrawType(e);
         ...
         function removeMaxRound(evt) {
            delete evt.max_round;
            if (evt.draw) delete evt.draw.max_round;
         }
         function removeStructure(evt) {
            delete evt.structure;
            delete evt.feed_base;
            delete evt.feed_rounds;
            delete evt.skip_rounds;
            delete evt.sequential;
         }

         function toggleStructureConfig() {
            let disp = (e.structure == 'feed') ? 'flex' : 'none';
            if (disp != 'flex') removeStructure(e);
            Array.from(container.draw_config.element.querySelectorAll('.feedconfig')).forEach(o=>o.style.display = disp);
         }
         ...
         function setQualificationConfig() {
            ...
            event_config = displayGen.configQualificationDraw({ container, e, feed_options, sequential_options, qualcounts: options });
         ...
         function setEliminationConfig() {
            ...
            if (e.structure == 'feed' && !supported_structures.feedin.elimination) removeStructure(e);
            ...
            configureStructureOptions(e, event_config);
         ...
         function configureStructureOptions(evt, event_config) {
            if (evt.feed_rounds == undefined) evt.feed_rounds = 0;
            event_config.feedrounds.ddlb = new dd.DropDown({ element: event_config.feedrounds.element, onChange: setFeedRounds });
            event_config.feedrounds.ddlb.setValue(evt.feed_rounds, 'white');
            event_config.skiprounds.ddlb = new dd.DropDown({ element: event_config.skiprounds.element, onChange: setSkipRounds });
            event_config.skiprounds.ddlb.setValue(evt.skip_rounds || 0, 'white');
            event_config.sequential.ddlb = new dd.DropDown({ element: event_config.sequential.element, onChange: setSkipRounds });
            event_config.sequential.ddlb.setValue(evt.sequential || 1, 'white');
         }

         function setConsolationConfig() {
            let linked = tfx.findEventByID(tournament, e.links['E']);
            let linked_not_feed = linked && linked.structure != 'feed';
            let structure_options = [{ key: lang.tr('draws.standard'), value: 'standard' } ];
            if (linked_not_feed && supported_structures.feedin.consolation) structure_options.push({ key: lang.tr('draws.feedin'), value: 'feed'});
            if (!linked_not_feed || (e.structure == 'feed' && !supported_structures.feedin.consolation)) removeStructure(e);

            event_config = displayGen.configTreeDraw({ container, e, structure_options, feed_options });
      ...
      function configureLocationAttributes(l) {
         ...
         if (env.locations.geoposition && (!coords.latitude || !coords.longitude)) coords = env.locations.geoposition.coords;
         ...
         if (map && state.edit) {
            map.on('click', function(e) {
               ...
               displayGen.okCancelMessage(message, () => setLatLng({ latitude: lat, longitude: lng }), ()=>displayGen.closeModal());
            });
         ...
         function promptGetUserPosition() {
            if (!state.edit) return;
            geoFx.getUserPosition({ successFx: setLatLng });
         }

         function getLatLng() {
            if (!state.edit) return;
            geoFx.getLatLng({ successFx: setLatLng });
         }

         function setLatLng({ latitude, longitude }) {
            l.latitude = latitude;
            l.longitude = longitude;
            attributes.latitude.element.value = latitude;
            attributes.longitude.element.value = longitude;
            attributes.latitude.element.style.background = 'white';
            attributes.longitude.element.style.background = 'white';
            ...
            marker.setLatLng(newLatLng);
            coms.emitTmx({ notice: `Setting Location: ${latitude},${longitude}`, version: env.version }); 
         }
      ...
      function consolationWildcards(evt, e) {
         ...
         let teams = pfx.teamOptionNames(wcteams, 'WC');
         let clickAction = (d, i) => {
      ...
      function eligibleOptions(evt, e) {
         ...
         let teams = pfx.teamOptionNames(losers, 'LL');
         let clickAction = (d, i) => {
      ...
      function configureEventSelections(e) {
         let corrupted = () => displayGen.okCancelMessage('Event is corrupted', () => displayGen.closeModal());
         if (tournament.type == 'dual') {
            coms.catchSync(configureDualEventSelections, corrupted)(e);
         } else {
            coms.catchSync(configureStandardEventSelections, corrupted)(e);
         }
      }
      ...
      function configureDualEventSelections(e) {
         ...
         // set up scoring options
         let scoring_options = [
            { key: 'Match', value: 'match' },
            { key: 'Set', value: 'set' }
         ];
         dd.attachDropDown({ 
            id: details.points_per.id, 
            options: scoring_options,
            label: ''
         });
         let per = (e.dual_points && e.dual_points.per) || 'match';
         details.points_per.ddlb = new dd.DropDown({ element: details.points_per.element, onChange: setDualMatchScoring });
         details.points_per.ddlb.setValue(per, 'white');

         function setDualMatchScoring(value) {
            if (!e.dual_points) e.dual_points = {};
            e.dual_points.per = value;
         ...
         function modifyMatchOrder(format, limit) {
            ...
            util.range(1, limit - counter + 1).forEach(() => matchorder.push({ format, value: 1 }));
         ...
         function updateDualMatchDetails(tab) {
            let dual_match_details = displayGen.displayDualMatchDetails({ container, e, matchorder: e.matchorder, edit: state.edit, tab });
            ...
            function renameMatchBlocks() {
               ...
               matchblocks.forEach((b, i) => {
                  ...
                  let format = matchname.innerText && matchname.innerText.toLowerCase().indexOf('doubles') >= 0 ? 'doubles' : 'singles';
            ...
            function handleDragEnd() { this.classList.remove('over'); }
            ...
            function updateHandlers() {
               ...
               if (!e.active) {
                  ...
                  domFx.addEventToClass('matchvalue', matchValue, container.detail_opponents.element, 'keyup');
                  domFx.addEventToClass('matchvalue', catchTab, container.detail_opponents.element, 'keydown');
                  domFx.addEventToClass('team_match', disableAllMatchValue, container.detail_opponents.element, 'focusout');
                  domFx.addEventToClass('team_match', enableMatchValue, container.detail_opponents.element, 'click');
                  domFx.addEventToClass('team_match', cycleGender, container.detail_opponents.element, 'contextmenu');
               }
            }

            function cycleGender(evt) {
               let team_match = domFx.getParent(evt.target, 'team_match');
      ...
      function configureStandardEventSelections(e) {
         ...
         domFx.addEventToClass('addall', addAll, container.detail_opponents.element);
         domFx.addEventToClass('removeall', removeAll, container.detail_opponents.element);
         domFx.addEventToClass('promoteall', promoteAll, container.detail_opponents.element);
      ...
      function generateDraw(e, delete_existing) { coms.catchSync(drawGeneration)(e, delete_existing); }
      function drawGeneration(e, delete_existing) {
         ...
         if (delete_existing) { console.log('deleting draw'); delete e.draw; }

         let total_players = (approved_opponents.length + (e.qualifiers || 0)) || e.feed_base;
         if (!total_players) {
            delete e.draw;
            e.draw_created = false;
            return;
         ...
         let consolation = () => {
            if (e.structure == 'feed') {
               ...
               let eligible_players = tfx.eligiblePlayers(tournament, e).players;
               let linked_elimination = tfx.findEventByID(tournament, e.links['E']);
               let linked_elimination_info = dfx.drawInfo(linked_elimination.draw);

               let opponents = eligible_players.map(player => {
                  player.elimination_position = linked_elimination_info.assigned_positions[player.puid];
                  return [player];
               });

               let ebp = linked_elimination_info.byes || [];
               let sbyes = linked_elimination_info.structural_byes || [];
               let bye_positions = ebp.concat(...sbyes).map(node => node.data && node.data.dp).filter(f=>f);

               /*
               let structural_byes = (linked_elimination_info.structural_byes && linked_elimination_info.structural_byes.map(node => node.data && node.data.dp).filter(f=>f)) || [];
               let elimination_bye_positions = !linked_elimination_info.byes ? [] : linked_elimination_info.byes.map(n=>n.data.dp);
               */

               // let bye_placements = elimination_bye_positions.concat(...structural_byes)
               let bye_placements = bye_positions
                  .map(elimination_bye_position => tfx.getConsolationPosition(linked_elimination, e.draw_size, 1, elimination_bye_position));

               bye_placements.forEach((position, i) => dfx.assignPosition({ node: e.draw, position, bye: i + 1 }));
               e.draw.bye_placments = bye_placements;

               opponents.forEach(opponent => tfx.placeConsolationOpponent(linked_elimination, e, opponent));
               dfx.advanceTeamsWithByes({ draw: e.draw });

               let first_round_positions = tfx.consolationOpponentsInRound(opponents, 1).length + bye_placements.length;

               if (first_round_positions == e.draw_size) {
                  e.draw.scoring_active = true;
                  drawCreated(e);
               }
         ...
         function emptyBrackets(num = 1) {
            return util.range(0, num).map(() => ({ puids: [], teams: [], players: [], matches: [] }));
         }
      ...
      function eventTournamentTeams(e) {
         ...
         domFx.addEventToClass('tt_click', changeGroup, container.event_details.element);
         ...
         function changeGroup(evt) {
            if (!state.edit || e.active) return;
            let grouping = domFx.getParent(evt.target, 'opponent_container').getAttribute('grouping');
            let elem = domFx.getParent(evt.target, 'tt_click');
      ...
      function eventPlayers(e) {
         ...
         if (tfx.isConsolationFeedIn(e)) eligible_players = [];
         ...
         function removeTeam(evt) {
            if (!state.edit || e.active) return;
            let grouping = domFx.getParent(evt.target, 'opponent_container').getAttribute('grouping');
            let elem = domFx.getParent(evt.target, 'team_click');
         ...
         function teamClickMenu(evt) {
            var grouping = domFx.getParent(evt.target, 'opponent_container').getAttribute('grouping');
            if (!state.edit || e.active) return;
            if (evt.ctrlKey || evt.shiftKey || grouping == 'approved') return removeTeam(evt);
            var elem = domFx.getParent(evt.target, 'team_click');
         ...
         domFx.addEventToClass('player_click', playerOptions, container.event_details.element);
         domFx.addEventToClass('team_click', teamClickMenu, container.event_details.element);
         domFx.addEventToClass('team_click', removeTeam, container.event_details.element, 'contextmenu');
         domFx.addEventToClass('player_click', changeGroup, container.event_details.element, 'contextmenu');
      ...
      function scheduleTab(specific_day) {
         ...
         let display_day = specific_day || displayed.schedule_day;
         if (display) { displaySchedule(display_day); }
      }
      ...
      function validDay(match) { if (match && match.schedule && match.schedule.day && !dateFx.validDate(match.schedule.day)) { delete match.schedule; } }
      ...
      function displaySchedule(currently_selected_day) {
         ...
         let date_range = dateFx.dateRange(tournament.start, tournament.end);

         // must be d so that i doesn't get passed...
         let formatted_date_range = date_range.map(d => dateFx.formatDate(dateFx.timeUTC(d)));

         if (currently_selected_day && formatted_date_range.indexOf(currently_selected_day) < 0) { currently_selected_day = undefined; }
         let date_range_ms = date_range.map(d => d.getTime());

         let all_matches = completed_matches.concat(...pending_matches, ...upcoming_matches);
         all_matches.forEach(validDay);

         let muid_key = Object.assign({}, ...all_matches.map(m=>({ [m.muid]: m })));
         let day_matches = all_matches;

         let ms = dateFx.offsetTime(dateFx.timeUTC(new Date()));

         let closest_schedule_ms = date_range_ms
            .reduce((p, c) => Math.abs(ms - c) <= Math.abs(ms - p) ? c : p, 0);

         let closest_match_day = all_matches
            .filter(m=>m.schedule && m.schedule.day)
            .map((m, i) => ({ i, ms: dateFx.offsetTime(new Date(m.schedule.day)) }))
         ...
         domFx.getParent(container.location_filter.element, 'schedule_options').style.display = (tournament.locations.length > 1) ? 'flex' : 'none';
         ...
         function filterSearchList() {
            ...
            search_list = all_matches
               ...
               .map(match => ({ value: match.muid, label: getOpponents(match) }) );
         }

         function getOpponents(match) {
            let team_players = match.team_players.map(team=>teamName(match, team, true));
            let potentials = match.potentials && match.potentials.map(potential=>teamName(match, potential, true, true)) || [];
            let possible = [].concat(...team_players, ...potentials, lang.tr('unk'), lang.tr('unk'));
            return possible.slice(0, 2).join(' - ');
         }

         function teamName(match, team, remove_diacritics, potentials) {
            if (potentials && match.potentials && match.potentials.length) {
               return match.potentials.map(p=>p && joinPotentials(p.map(o=>o && determine(o, Object.keys(o)) || lang.tr('unk')))).filter(f=>f);
            } else {
               return determine(match.players, team);
            }

            function joinPotentials(potentials) { return bothUnknown(potentials) ? lang.tr('unk') : potentials.join(` ${lang.tr('or')} `); }
            function bothUnknown(opponents) { return opponents && opponents.length == 2 && opponents[0] == lang.tr('unk') && opponents[1] == lang.tr('unk'); }
            function determine(players, team) { return (team.length == 1) ? singles(players, team) : doubles(players, team); }
            function singles(players, team) {
               let p = players[team[0]];
               if (!p) return '';
               if (!p.last_name || !p.first_name) return p.qualifier ? lang.tr('qualifier') : '';
               let club = p.club_code ? ` (${p.club_code})` : '';
               let full_name = `${stringFx.normalizeName(p.first_name, remove_diacritics)} ${stringFx.normalizeName(p.last_name, remove_diacritics).toUpperCase()}`; 
               return `${full_name}${club}`;
            }
            function doubles(players, team) {
               return team
                  .map(t => !players[t] ? lang.tr('unk') : stringFx.normalizeName(players[t].last_name, remove_diacritics).toUpperCase()).join('/');
            }
         }
         ...
         function displayPending(update_round_names=true) {
            ...
            let srtd_unscheduled = sfx.sortedUnscheduled(tournament, unscheduled, false);
            displayGen.scheduleTeams({ unscheduled: srtd_unscheduled, element: container.unscheduled.element });

            domFx.addEventToClass('dragUnscheduled', dragUnscheduled, container.unscheduled.element, 'dragstart');
         ...
         function displayScheduleGrid(tournament, container) {
            // must have enough rows to show any matches which may have been scheduled beyond the current settings range
            let min_rows = Math.max(...day_matches.map(m=>m.schedule && m.schedule.oop_round).filter(f=>f));
            let upper_range = Math.max(min_rows, util.parseInt(env.schedule.max_matches_per_court) || 16);
            let oop_rounds = util.range(1, upper_range + 1);
            ...
            domFx.addEventToClass('findmatch', showSearch, container.schedule.element, 'click');
            domFx.addEventToClass('opponentsearch', (e)=>e.stopPropagation(), container.schedule.element, 'click');

            domFx.addEventToClass('dragdrop', dragStart, container.schedule.element, 'dragstart');
            domFx.addEventToClass('dragdrop', safeDrop, container.schedule.element, 'drop');

            domFx.addEventToClass('oop_round', roundContext, container.schedule.element, 'contextmenu');
            domFx.addEventToClass('oop_round', roundContext, container.schedule.element, 'click');
            domFx.addEventToClass('schedule_box', gridContext, container.schedule.element, 'contextmenu');
            domFx.addEventToClass('schedule_box', gridClick, container.schedule.element, 'click');
            scheduleActions();
            checkConflicts(day_matches);
         }

         domFx.addEventToClass('dropremove', dropRemove, document, 'drop');
         ...
         function populateGridCell(target, muid, match) {
            ...
            if (muid) {
               ...
               let sb = sfx.scheduleBox({ match, editable: true, options: env.schedule });
               target.innerHTML = sb.innerHTML;
               target.style.background = sb.background;
               sfx.scaleTeams(target);
            } else {
               target.setAttribute('muid', '');
               target.setAttribute('draggable', 'false');
               target.innerHTML = sfx.emptyOOPround(true);
               domFx.addEventToClass('findmatch', showSearch, target, 'click');
               domFx.addEventToClass('opponentsearch', (e)=>e.stopPropagation(), target, 'click');
            }
         }
         ...
         function safeReturnToUnscheduled(match, element) { return coms.catchSync(returnToUnscheduled)(match, element); }
         function returnToUnscheduled(match, element) {
            ...
            element.innerHTML = sfx.emptyOOPround(true);
            domFx.addEventToClass('findmatch', showSearch, element, 'click');
            domFx.addEventToClass('opponentsearch', (e)=>e.stopPropagation(), element, 'click');
         ...
         function showSearch(evt) {
            ...
            if (display_state == 'none') {
               ...
               let matchSelected = (muid) => {
                  ...
                  safeScheduleMatch(muid, interrupted);
               };
               opponent_search.addEventListener("awesomplete-selectcomplete", function() { selection_flag = true; matchSelected(this.value); }, false);
               ...
               evt.target.innerHTML = sfx.opponentSearch();
            }

            // wrapper for scheduleMatch(muid, interrupted);
            function safeScheduleMatch(muid, interrupted) {
               return coms.catchSync(scheduleMatch)(muid, interrupted);
            }

            function scheduleMatch(source_muid, interrupted) {
               let target = domFx.getParent(evt.target, 'schedule_box');
         ...
         function dragUnscheduled(ev) {
            let target = domFx.getParent(ev.target, 'dragUnscheduled');
            var id = target.id;
         ...
         function dropRemove(ev) {
            ...
            safeReturnToUnscheduled(source_match, source);
            saveTournament(tournament);
         }

         function safeDrop(ev) { coms.catchSync(drop)(ev); }
         function drop(ev) {
            ev.preventDefault();
            let itemtype = ev.dataTransfer.getData("itemtype");
            let itemid = ev.dataTransfer.getData("itemid");
         ...
         function identifyRound(ev) {
            let target = domFx.getParent(ev.target, 'oop_round');
            let oop_round = parseInt(target.getAttribute('oop_round'));
            return { oop_round };
         }

         function roundContext(ev) {
            let modifier = ev.ctrlKey || ev.shiftKey;
            if (!state.edit || (!schedulingActive() && !modifier)) return;
            ...
            function modifySchedules(choice) {
         ...
         function scheduleGridMatch({ evt, target }) {
            ...
            function assessPenalty(penalty) {
               ...
               function playerPenalty(player, index) {
            ...
            function modifySchedule(choice) {
               if (choice.key == 'matchtime') {
                  ...
                  if (!target) target = domFx.getParent(evt.target, 'schedule_box');
                  safeReturnToUnscheduled(match, target);
                  return;
               }
            }
      ...
      function locationList(displayed_location) {
         ...
         eventManager.register('displayLocationDetails', 'tap', displayLocationDetails);

         function displayLocationDetails(target) {
            let clicked_location = domFx.getParent(target, 'location');
            let class_list = clicked_location && clicked_location.classList;
            let index = clicked_location && clicked_location.getAttribute('index');
            if (class_list && class_list.contains('highlight_listitem')) {
               closeLocationDetails();
               locationList();
            } else {
               ...
               class_list.add('highlight_listitem');
               if (index) displayLocation({ location: tournament.locations[index], index });
            }
         }
               ...
               actions.select('.save')
                  ...
                  .on('click', () => { 
                     ...
                     if (!l.abbreviation || !l.name || !l.address) {
                        return displayGen.popUpMessage(`Location details are incomplete!`);
                     }
      ...
      function playersTab({ doubles } = {}) {
         ...
         function categoryChanged() { playersTab(); }
         ...
         displayGen.displayTournamentPlayers({ container, tournament, players: t_players, filters, ratings_type, edit: state.edit, doubles });
         ...
         function tournamentPlayerContext(evt) {
            ...
            let element = domFx.getParent(evt.target, 'player_click');
            var mouse = { x: evt.clientX, y: evt.clientY };
            tournamentPlayerContextOptions(element, mouse);
         }
         ...
         function tournamentPlayerContextOptions(element, mouse) {
            ...
            function selectionMade(choice) {
               if (choice.key == 'edit') {
                  ...
                  let caption = `<p>${lang.tr('delete')} ${lang.tr('ply')}:</p> <p>${clicked_player.full_name}</p>`;
                  displayGen.okCancelMessage(caption, deletePlayer, () => displayGen.closeModal());
               } else if (choice.key == 'identify' || choice.key == 'merge') {
               ...
               function identifyValidPlayer(valid_players) {
                  ...
                  let selection_flag = false;
                  modal.search.element.addEventListener("awesomplete-selectcomplete", function() { selection_flag = true; setValue(); }, false);
                  ...
                  function confirmIdentity(new_player_data) {
                     ...
                     new_player_data.birth = dateFx.formatDate(new_player_data.birth);
                     ...
                     function changePlayerIdentity() {
                        db.findClub(new_player_data.club + '').then(club => {
                           if (club && club.code) new_player_data.club_code = club.code;
                           finish();
                        });

                        function finish() {
                           displayGen.closeModal();
                           tfx.replaceTournamentPlayer({ tournament, existing_player: clicked_player, new_player_data, replace_all: choice.key != 'merge' });
                           saveFx();
                        }
                     }
                  }
         ...
         let signInState = (evt) => {
            // if modifying rankings, disable!
            if (!state.edit || state.manual_ranking) return;
            ...
            if (state.edit && o.sign_in.rapid && !withdrawn && registration) {
               if (clicked_player) {
                  if (clicked_player.signed_in) {
                     if (approved || active) {
                        let saveFx = () => { saveTournament(tournament); playersTab(); };
            ...
            function displayIrregular(player) {
               if (state.edit) {
                  player.signed_in = false;
                  saveTournament(tournament);
               } else {
         ...
         let playerByPUID = (puid) => tournament.players
            .reduce((p, c) => {
               if (c.puid == puid) { p = c; }
               return p;
            }, undefined);
                  ...
                  if (changeable) {
                     if (value.indexOf('{') == 0) {
                        ...
                        if (doubles) {
                           ...
                           checkDuplicateRankings({ subrank: !doubles });
                        }
                     }
                  } else {
                     if (doubles) {
         ...
         domFx.addEventToClass('manualrank', catchTab, container.players.element, 'keydown');
         domFx.addEventToClass('manualrating', catchTab, container.players.element, 'keydown');
         domFx.addEventToClass('subrank', catchTab, container.players.element, 'keydown');

         domFx.addEventToClass('tournamentPlayers', tournamentPlayers, container.players.element);
         domFx.addEventToClass('tournamentPlayers', tournamentPlayers, container.players.element, 'contextmenu');

         domFx.addEventToClass('player_click', signInState, container.players.element);
         domFx.addEventToClass('player_click', tournamentPlayerContext, container.players.element, 'contextmenu');
         domFx.addEventToClass('ranksub', stopPropagation, container.players.element);
         domFx.addEventToClass('rankentry', stopPropagation, container.players.element);
         domFx.addEventToClass('ratingentry', stopPropagation, container.players.element);
         domFx.addEventToClass('manualrank', rankEntryKey, container.players.element, 'keyup');
         domFx.addEventToClass('manualrating', ratingEntryKey, container.players.element, 'keyup');
         domFx.addEventToClass('subrank', subRankKey, container.players.element, 'keyup');

         domFx.addEventToClass('rankbyrating', rankByRating, container.players.element);
         domFx.addEventToClass('rankbyrating', rankByRating, container.players.element, 'contextmenu');
         eventManager.holdActions.rankByRating = rankByRating;

         domFx.addEventToClass('playerRanking', playerRanking, container.players.element);
         domFx.addEventToClass('playerRanking', playerRanking, container.players.element, 'contextmenu');
      ...
      function tournamentPlayers() {
         if (state.edit && tournament.players && tournament.players.length) {
            ...
            displayGen.okCancelMessage(message, addPlayers, () => { displayGen.closeModal(); modalViews.closeModal(); });
         }
         function addPlayers() {
      ...
      function displayedPlayers() {
         ...
         if (env.documentation.hoverhelp) {
            document.querySelector('.' + classes.ranking_order).classList[opacity ? 'add' : 'remove'](displayGen.infoleft || 'stub');
         }
         return displayed_players;
      }

      function checkDuplicateRankings({ subrank }) {
      ...
      function tournamentPoints(tournament, mz) {
         ...
         var points_date = dateFx.offsetDate(tournament_date);
      ...
      function matchesTab() {
         let { total_matches, completed_matches, pending_matches } = mfx.tournamentEventMatches({ tournament, source: true });
         ...
         domFx.addEventToClass('player_click', pfx.playerClicked, container.matches.element);
         domFx.addEventToClass('player_click', playerInMatchContext, container.matches.element, 'contextmenu');
         ...
         function playerInMatchContext(evt) {
            let puid = evt.target.getAttribute('puid');
            let row = domFx.getParent(evt.target, 'matchrow');
         ...
         domFx.addEventToClass('cell_singles', matchClick, container.matches.element);
         domFx.addEventToClass('cell_doubles', matchClick, container.matches.element);
         domFx.addEventToClass('cell_singles', scoreClick, container.matches.element, 'contextmenu');
         domFx.addEventToClass('cell_doubles', scoreClick, container.matches.element, 'contextmenu');
      ...
      function displayDraw({ evt }) { coms.catchSync(displayDrawType)({evt}); }
      ...
      function enableDrawActions() {
         ...
         // let consolation_feed_in = tfx.isConsolationFeedIn(displayed.draw_event);
      ...
      function removeDirectionalPlayer(e, target_draw, losing_team_ids, linked_info) {
         tfx.removeDirectionalPlayer(tournament, e, target_draw, losing_team_ids, linked_info);
      }
      ...
      function displayDualMatches(dual_match, dual_teams) {
         ...
         if (!dual_teams || !dual_teams.length || dual_teams.length != 2) return;
         ...
         function dualTeamClick(elem, mouse) {
            ...
            let options = [lang.tr('scoring.retire'), lang.tr('scoring.walkover'), lang.tr('scoring.default'), lang.tr('clr')];
            displayGen.svgModal({ x: mouse.x, y: mouse.y, options, callback: clickAction });

            function clickAction(d, i) {
               let draw_position = dual_teams[1-team_index].draw_position;
               if (i == 0) {
                  ...
                  tfx.advanceDualMatchPosition({ e, dual_match, dual_teams });
                  console.log('remove any added scoring information and recalculate');
               }
      ...
      function displayOrderedMatches(e, dual_match, dual_teams) {
         ...
         domFx.addEventToClass('dual_match', dualMatchClick);
         domFx.addEventToClass('dual_match_team_name', dualMatchOpponent);

         // capture CHANGE event for all DDLB
         domFx.addEventToClass('dual_select', (evt) => evt.stopPropagation());
         domFx.addEventToClass('dual_select', opponentSelected, document, 'change');
         ...
         function dualMatchOpponent(evt) {
            if (state.edit) {
               evt.stopPropagation();
               let dm_elem = domFx.getParent(evt.target, 'dual_match');
               ...
               let team_name = domFx.getParent(evt.target, 'dual_match_team_name');
         ...
         function opponentSelected(evt) {
            let elem = evt.target;
            let dm_elem = domFx.getParent(elem, 'dual_match');
            let match_muid = dm_elem && dm_elem.getAttribute('muid');
            let match = matches.reduce((p, c) => c.match.muid == match_muid ? c : p, undefined);

            let team_name = domFx.getParent(elem, 'dual_match_team_name');
      ...
      function processResult(tournament, e, result) {
         ...
         if (result.devmessage) {
            displayGen.popUpMessage(result.devmessage);
            return result;
         }
      ...
      function genEventDraw(value) {
         ...
         let total_players = (((e.approved && e.approved.length) || 0) + (e.qualifiers || 0)) || e.feed_base;
         if (!total_players || (e.draw_type == 'R' && e.approved.length < 3)) {
            e.draw_created = false;
            delete e.draw;
         } else {
            displayDraw({ evt: e });
         ...
         function RRgroupName(d) {
            if (state.edit && !d.row && !d.column) {
               ...
               rr_name_obj.search_field.element.addEventListener("keyup", function(e) { 
                  if (e.which == 13) { submitRRname(rr_name_obj.search_field.element.value); }
               });
            }
         }
         ...
         function rrScoreAction(d) {
            if (state.edit) {
               ...
               if (d.match && d.match.score) {
                  let clickAction = (c, i) => { if (i == 0) removeOption(d); };
                  cMenu({ selector, coords, options, clickAction });
               }
            }
         ...
         function treePositionClick(d, coords) {
            ...
            // don't go any further if the draw is incomplete... unless scoring is active (consolation feed-in only)
         ...
         function getSeedTeams(seed_group) {
            ...
            let teams = pfx.teamOptionNames(remaining.map(r=>current_draw.seeded_teams[r]).filter(f=>f));
         ...
         function seedAssignment(draw, seed_group, position, value) {
            ...
            let { remaining, positions } = getSeedTeams(seed_group);
         ...
         function rrPlacePlayer(placement, pobj, info, d, coords) {
            ...
            let list = unplaced_teams.map(team => { 
               let player = team[0];
               let label = `${stringFx.normalizeName([player.first_name, player.last_name].join(' '))}`;
            ...
            pobj.player_search.element
               .addEventListener("awesomplete-selectcomplete", function() { selection_flag = true; selectPlayer(this.value); }, false);
            ...
            function playerIndex(evt) {
               ...
               let invalidEntry = () => pobj.player_index.element.value = '';
         ...
         function placeTreeDrawPlayer(d, coords) {
            if (tfx.isConsolationFeedIn(e)) return;
            ...
            let selection_flag = false;
            let opponentLabel = (opponent) => stringFx.normalizeName(opponent.name || [opponent.first_name, opponent.last_name].join(' '));
            let list = unplaced_teams.map(team => { 
         ...
         function rrContextActions(d, draw, info, coords) {
            ...
            function placeTeam(unplaced_teams, placement, info) {
               ...
               let options = pfx.teamOptionNames(unplaced_teams);
               let clickAction = (d, i) => {
            ...
            function assignedRRoptions(placement, cell, draw) {
               ...
               let clickAction = (d) => {
                  ...
                  if (d.key == 'alternate') {
                     return rrAlternates({ placement, unseeded_position, coords, draw, options: alternates, entry: 'A' });
                  }
               };
         ...
         function rrAlternates({ placement, unseeded_position, coords, draw, options, entry }) {
            let teams = pfx.teamOptionNames(options);
            let clickAction = (d, i) => {
         ...
         function contextPopUp(d, coords) {
            ...
            if (!state.edit) {
               ...
               coms.catchSync(drawNotActiveContextClick)(d, coords);
            } else {
               if (!tfx.isTeam(tournament)) {
                  coms.catchSync(drawActiveContextClick)(d, coords);
               }
            }
         }
         ...
         function findSeedGroup(draw, seed) {
            return draw && draw.seed_placements && draw.seed_placements.reduce((p, s) => s.range.indexOf(seed) >= 0 ? s : p, undefined);
         }
         ...
         function assignedPlayerOptions({ position, node, coords, info, draw, seed }) {
            let unfinished = info.unassigned.length; // this should be e.active ?
            if (displayed.draw_event.structure != 'feed' && seed && seed < 3 && unfinished) {
               displayGen.popUpMessage('Cannot Remove 1st/2nd Seed');
               return;
            }
            let teams = pfx.teamOptionNames([node.data.team]);

            let compass_direction = draw.direction && draw.direction != 'east';
            let unfinished_options = compass_direction ? [] : teams.map(t => `${lang.tr('draws.remove')}: ` + t);
            ...
            let clickAction = (d) => {
               if (unfinished) {
                  ...
                  if (seed) {
                     ...
                     if (seed_group) {
                        seed_group.placements = seed_group.placements.filter(p=>p.seed != seed);
                     } else {
                  ...
                  if (d.key) {
                     if (d.key == 'swap') {
                        ...
                        displayGen.escapeModal();
                        swap.entry_field.element.addEventListener('click', () => removeEntryField(swap));
                        swap.new_position.element.addEventListener('keyup', evt => playerIndex(evt, swap) , false);
            ...
            function removeEntryField(swap) {
               d3.select(swap.entry_field.element).remove();
               document.body.style.overflow = null;
               displayGen.escapeFx = undefined;
            }

            function playerIndex(evt, swap) {
         ...
         function drawActiveContextClick(d, coords) {
            ...
            var linked = tfx.findEventByID(tournament, e.links['E']) || tfx.findEventByID(tournament, e.links['S']) || tfx.findEventByID(tournament, e.links['C']);
            ...
            if (e.draw.compass && d.data.match && d.data.match.loser) {
               ...
               if (target_draw) {
                  ...
                  */
                  ...
                  active_in_linked = losing_team_ids && tfx.teamActiveInLinked(linked_info, losing_team_ids);
               }
            }
            ...
            if (qualification && match_score && qualified && linked && linked.draw && d.data.team) {
               ...
               */
               ...
               active_in_linked = tfx.teamActiveInLinked(linked_info, team_ids);
            }

            if (linked && tfx.isConsolationFeedIn(linked)) {
               linked_info = dfx.drawInfo(linked.draw);
               let losing_team_ids = d.data.match && d.data.match.loser && d.data.match.loser.map(l=>l.id);
               active_in_linked = losing_team_ids && tfx.teamActiveInLinked(linked_info, losing_team_ids);
            }
            ...
            if (d.data.bye) {
               ...
               let callback = () => {
                  mfx.eventMatches(e, tournament);
                  if (tfx.isConsolationFeedIn(linked)) {
                     console.log('bye must be removed from consolation draw and advanced player un-advanced');
                  }
               };
               ...
               if (tfx.isConsolationFeedIn(linked)) {
                  console.log('check whether bye is ACTIVE (advanced players is active) in consolation draw');
               }

               if (possible_to_replace && (alternates.length || losers.length)) {
                  let options = [{ option: lang.tr('draws.alternate'), key: 'alt' }];
                  if (losers.length) { options.push({ option: lang.tr('draws.luckyloser'), key: 'lucky' }); }
                  let clickAction = (k) => {
                     if (k.key == 'alt') {
               ...
               let clickAction = () => {
                  ...
                  if (tfx.isConsolationFeedIn(linked)) {
                     let losing_team_ids = d.data.match.loser.map(m=>m.id);
                     tfx.removeConsolationTeam(tournament, e, linked.draw, losing_team_ids, linked_info);
                  }
               ...
               assignedPlayerOptions({ position, node: d, coords, info, draw: current_draw, seed: d.data.team[0].seed });
            }
         }
         ...
         function drawNotActiveContextClick(d, coords) {
            ...
            if (!d.height && d.data && d.data.dp && !d.data.team) {
               ...
               let seed_group = dfx.nextSeedGroup({ draw: current_draw });
               if (seed_group) {
                  assignSeededPosition({ position, draw: current_draw, seed_group, coords });
               } else {
                  if (o.byes_with_unseeded) {
                     assignUnseededPosition({ position, draw: current_draw, coords });
                  } else {
               ...
               assignedQBOptions({ position, node: d, coords, draw: current_draw, qualifier: true });
            } else if (d.height == 0 && d.data.bye && !displayed.draw_event.active) {
               assignedQBOptions({ position, node: d, coords, draw: current_draw, bye: true });
            ...
            function assignedQBOptions({ position, node, coords, draw, qualifier, bye }) {
               let info = dfx.drawInfo(draw);
               let unfinished = info.unassigned.length;
               let what = qualifier ? 'Qualifier' : bye ? 'BYE': '?';
            ...
            function assignSeededPosition({ position, draw, seed_group, coords }) {
            ...
            function assignUnseededPosition({ position, draw, coords }) {
               let info = dfx.drawInfo(draw);
               ...
               var unplaced = draw.unseeded_teams.filter(team => placements.indexOf(team[0].id) < 0);
               ...
               let teams = pfx.teamOptionNames(unplaced_teams);
      ...
      function addUmpire(match, context) {
         ...
         uobj.search_field.element.addEventListener("awesomplete-selectcomplete", () => {
            selection_flag = true;
            selectUmpire(this.value);
         }, false);
      ...
      function genGroupDraw(value) {
         ...
         if (['msm', 'wsm', 'md', 'wd'].indexOf(value) >= 0) {
            let evt = { draw: legacyProcess.generateDrawTree(match_groups[value]), draw_type: 'E' };
            displayDraw({ evt });
         } else if (['msq', 'wsq'].indexOf(value) >= 0) {
            let tree = legacyProcess.generateDrawTree(match_groups[value], 'Q');
            let evt = { draw: tree, draw_type: 'Q' };
            displayDraw({ evt });
         } else if (['msrr', 'wsrr'].indexOf(value) >= 0) {
            let brackets = legacyProcess.generateDrawBrackets(match_groups[value]);
      ...
      function tournamentTab() {
         ...
         let tournament_date = tournament && (tournament.points_date || tournament.end);
         let points_date = dateFx.offsetDate(tournament_date);
      ...
      function legacyTournamentOptions() {
         ...
         legacyProcess.legacyTournament(tournament, container);
      }
      ...
      function tournamentOptions(days) {
         ...
         let day_times = days.map(d=>dateFx.offsetTime(d));
         let max_start = Math.min(...day_times);
         // let min_end = Math.max(...day_times);

         let start = dateFx.offsetDate(tournament.start);
         let end = dateFx.offsetDate(tournament.end);
         ...
         function updateEndDate() {
            tournament.end = dateFx.timeUTC(end);
         ...
         container.start_date.element.addEventListener('keyup', () => nextFieldFocus('start_date'));
         container.end_date.element.addEventListener('keyup', () => nextFieldFocus('end_date'));
      }
      ...
      function drawsTab() {
         ...
         function eventChange(value) {
            if (event_draws) {
               ...
               let draw = displayed.draw_event && displayed.draw_event.draw;
               if (tfx.isTeam(tournament) && draw && draw.opponents && draw.opponents.length == 2) {
                  teamTournamentScore({ data: draw });
               }
      ...
      function getDualTeams(dual_match) {
         ...
         dual_teams
            .forEach(dt => {
               ...
               dt.players = team && team.players && Object.assign({}, ...Object.keys(team.players).map(k => ({ [k]: team.players[k] }) ));
            });
         return dual_teams.filter(dt => dt.players);
      }
      ...
      if (selected_tab && tab_ref[selected_tab] != undefined) displayTab(tab_ref[selected_tab]);
   ...
   fx.calcPlayerPoints = calcPlayerPoints;
   function calcPlayerPoints({ date, tournament, matches, container, filters=[] }) {
      ...
      let rankings = mz.length ? legacyProcess.legacyTournamentOpts(undefined, container) : {};
   ...
   function addAcceptedRankings(container, tournament, matches, category) {
      ...
      let rankings = matches.length ? legacyProcess.legacyTournamentOpts(undefined, container) : {};
   ...
   function checkAllPlayerPUIDs(players) {
      return new Promise((resolve, reject) => Promise.all(players.map(checkPlayerPUID)).then(resolve, reject));
   }
   ...
   function displayTournamentPoints(container, tournament, points={}, filters) {
      ...
      if (pts) {
         ...
         let pp = (evt) => pfx.displayPlayerProfile({ puid: domFx.getParent(evt.target, 'point_row').getAttribute('puid') }).then(()=>{}, ()=>{});
         domFx.addEventToClass('point_row', pp, container.points.element);
      }
   }

   function attachFilterToggles(classes, filterFx = console.log) {
      domFx.addEventToClass(classes.filter_m, () => filterFx('M'));
      domFx.addEventToClass(classes.filter_w, () => filterFx('W'));
      domFx.addEventToClass(classes.filter_s, () => filterFx('S'));
      domFx.addEventToClass(classes.filter_d, () => filterFx('D'));
   }
