      ...
      return new Promise( (resolve, reject) => {
         ...
         function generate(players) {
            ...
            let hts_points = points.map(point => {
               ...
               let eligible_categories = rankCalc.eligibleCategories({ birth_year: born, calc_date: new Date() });
               ...
               return {
                  ...
                  spol: point.gender,
                  tip: tournament_type,
                  datum_obracuna: util.formatDate(point.date, '-'),
                  kalendarID: point.tuid.match(/\d+/g)[0],
                  sezona: new Date(point.date).getFullYear(),
   ...
   hts.downloadHTSformattedPoints = ({ tuid, points=[], group_size = 1000 }) => {
      return new Promise( (resolve, reject) => {
         ...
         function formatPoints(points) {
            ...
            convertPointsToHTS(points).then(exportPoints);
         }
         ...
         function exportPoints(points) {
            ...
            while (cursor < points.length) {
               ...
               exportFx.downloadText(`hts_format_points${tuid_string}.csv`, csv);
               cursor += group_size;
            }
         }
      });
